...
if(widthAll > 1025){
    ...
    window.addEventListener('scroll', function(ev) {
        ...
        if(distanceToTop <= 415 && checkScrool == false || distanceToTop <= 448 && checkScrool == true){
            checkScrool = true;
            var x1 = -45;
            // $(".header_topbar").css({"background" : "black", "height" : '50px', "z-index" : 4});
            $(".header_topbar").css({"height" : '', "z-index" : 4});
        ...
        }else if(distanceToTop >= 448 && checkScrool == true){
            $(".header_topbar").css({"background" : "", "height" : '', "z-index" : 5});
            $(".header_navbar").css({"position" : "absolute", "height" : '95px', "background" : "linear-gradient(#25211e, rgba(37,33,30,0))","padding-top" : '55px', "z-index" : 4});
            blackNav = false;
            // console.log(blackNav);
        }
